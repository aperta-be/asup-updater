...
git clone --branch master --progress --verbose git@gitlab.dazzle.be:dazzle/brrc-geoportal.git .
...
if [[ $(composer update --with-dependencies) == *"Could not apply patch!"* ]]; then echo -e "# \e[1;31mCould not apply patch!\e[0m"; exit 1; else echo "All updates OK"; fi
