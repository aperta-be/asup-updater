function report_mattermost() {
  ...
  # TODO: Check the response code and report if it is not 200
  # curl -i --silent -o /dev/null -X POST -H 'Content-Type: application/json' -d "{\"username\": \"ASUP\", \"text\": \"$REPORT $OUTDATED_TABLE\"}" "$MATTERMOST_HOOK"
  curl --silent -o /dev/null -X POST -H 'Content-Type: application/json' -d "{\"username\": \"ASUP\", \"text\": \"$REPORT $OUTDATED_TABLE\"}" "$MATTERMOST_HOOK"
}
