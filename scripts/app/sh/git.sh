...
function git_get_code() {
  ...
  GIT_CLONE_URL_WITH_TOKEN=$(echo $GIT_CLONE_URL | sed "s#https\?://#https://${GIT_APERTA_USER}:${GIT_APERTA_TOKEN}@#g" | tr -d \")
...
function git_write_vars_vcs() {
  cat >/code/api/variables.php <<EOL
    ...
    const GIT_APERTA_USER = '$GIT_APERTA_USER';
    const GIT_APERTA_TOKEN = '$GIT_APERTA_TOKEN';
